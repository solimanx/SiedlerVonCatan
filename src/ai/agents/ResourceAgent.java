...
public class ResourceAgent {
	...
	private ArrayList<StreetSet> myStreetSets = new ArrayList<StreetSet>(); //evtl. import ändern
	...
	public Edge getBestStreet(){
		...
		ArrayList<Integer> edgeWeighting = new ArrayList<Integer>();
		...
		return null; //edgeSuggestion.get(index of max(edgeWeighting)
	}
	
	
	/**
	 * gets the possible trading route extensions for the AI. 
	 * @return ArrayList<Object> with first: streetSet; second: ArrayList<Edge> best street suggestions
	 */
			...
			//berechne zuerst alle endständigen Straßen dieses Street Sets
				...
				if (currEndingStreets.isEmpty() || currStreetSet.getHasCircle()) {
					...
					for (int j = 0; j <greatestValue.size();j++){
						...
						// für den Fall dass es einen echten Kreis gibt; füge alle hinzu
						// liefert falsches Ergebnis bei Doppelkreis, da keine Straße das Strassenset 
				...
				//Speichere Index i, falls die längste Straße des Spielers in aktuellem
				// Street Set ist. 
				if (gV > maxIndex[1]){
					maxIndex[0] = i;
					maxIndex[1] = gV;
