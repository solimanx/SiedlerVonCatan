...
public class GameViewController implements Initializable {
	...
	void handlePlayCardButton(ActionEvent event) throws IOException {
		...
		Parent root;
		try {
			root = loader.load(getClass().getResource("/view/devcardview/DevCardView.fxml").openStream());
			Scene scene = new Scene(root);
			scene.getStylesheets().add("/textures/" + theme + ".css");
	...
	private void setRobberLossState() {
		...
		Parent root;
		try {
			root = loader.load(getClass().getResource("/view/robberview/GiveResources.fxml").openStream());
			Scene scene = new Scene(root);
			scene.getStylesheets().add("/textures/" + theme + ".css");
	...
	private class ViewBoardFactory {
		...
		public ImageView drawVillage(double[] center) {
			...
			// center[1] - 10);
			Image villageImage = new Image("/textures/" + theme + "/village.png");
			...
			villageImageView.setEffect(shadow);
