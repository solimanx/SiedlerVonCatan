...
public class TradeController {
	...
	public void offerToAllPlayers(int offeringPlayerId,int[] resourcesToGet,int[] resourcesToGive){
		if (playerModels[offeringPlayerId].getPlayerState() != PlayerState.TRADING){
			//networkController.sendErrorMessageToPlayer(offeringPlayerId,"Offering not allowed");
		} else{
			if (gameLogic.checkPlayerResources(offeringPlayerId, resourcesToGive)){
				...
				//networkController.sendErrorMessageToPlayer(offeringPlayerId,"not enough Resources");
			}

		}
	}
	
	public void offerToPlayingPlayer(int offeringPlayerId,int receivingPlayerId,int[] resourcesToGet,int[] resourcesToGive){
